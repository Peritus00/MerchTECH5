...
const InlineMediaPlayer: React.FC<InlineMediaPlayerProps> = ({ 
  ...
  // Enhanced video detection - check filename extension and multiple type properties
  const filename = (file.filename || file.title || '').toLowerCase();
  const hasVideoExtension = /\.(mp4|mov|avi|webm|mkv|flv|wmv|m4v)$/i.test(filename);
  const isVideo = 
    file.type === 'video' || 
    file.fileType === 'video' || 
